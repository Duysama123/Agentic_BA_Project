...
def generate_srs_pdf(ba_data_json: str, output_path: str):
    ...
    pdf.cell(0, 20, data.get('system_name', 'System Specification'), ln=True, align='C')
    pdf.set_font('Helvetica', '', 12)
    pdf.cell(0, 10, f"Version: {data.get('version', '1.0')}", ln=True, align='C')
    pdf.ln(20)
    ...
    pdf.multi_cell(0, 8, str(intro.get('purpose', '')))
    
    pdf.set_font('Helvetica', 'B', 12)
    pdf.cell(0, 10, '1.2 Project Scope', ln=True)
    pdf.set_font('Helvetica', '', 11)
    pdf.multi_cell(0, 8, str(intro.get('project_scope', '')))
    pdf.ln(5)
    ...
    pdf.set_font('Helvetica', '', 11)
    pdf.multi_cell(0, 8, str(overall.get('product_perspective', '')))
    
    pdf.set_font('Helvetica', 'B', 12)
    pdf.cell(0, 10, '2.2 Operating Environment', ln=True)
    pdf.set_font('Helvetica', '', 11)
    pdf.multi_cell(0, 8, str(overall.get('operating_environment', '')))
    pdf.ln(5)
    ...
    for fr in reqs:
        pdf.set_font('Helvetica', 'B', 12)
        pdf.cell(0, 10, f"[{fr.get('id')}] {fr.get('name')}", ln=True)
        pdf.set_font('Helvetica', '', 11)
        pdf.multi_cell(0, 8, f"Actor: {fr.get('actor')}")
        pdf.multi_cell(0, 8, f"Description: {fr.get('description')}")
    ...
    for nfr in nfrs:
        pdf.set_font('Helvetica', 'B', 12)
        pdf.cell(0, 10, f"[{nfr.get('id')}] {nfr.get('type')}", ln=True)
        pdf.set_font('Helvetica', '', 11)
        pdf.multi_cell(0, 8, str(nfr.get('description', '')))
        pdf.ln(2)
